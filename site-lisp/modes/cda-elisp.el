(require 'cda-elisp-style-guide)

;;* Misc
;;;###autoload
(defun cda-emacs-lisp-hook ()
  (ignore-errors
...
(define-key lisp-mode-shared-map (kbd "β") 'cda-package-symbol)
...
(defun cda-lisp-interaction-hook ()
  (lispy-mode 1)
  (company-mode 1))

;;* Add the last eval result to "M-y"
(defvar cda-last-eval-expression-result "")

(defun cda-last-eval-expression-result ()
  (list cda-last-eval-expression-result))

(defun cda-eval-expression (orig-fun &rest args)
  (setq cda-last-eval-expression-result
        (prin1-to-string
         (apply orig-fun args))))
(cda-advice-add 'eval-expression :around #'cda-eval-expression)

(ivy-set-sources
 'counsel-yank-pop
 '((original-source)
   (cda-last-eval-expression-result)))
...
(defun cda-edebug-hook ()
  ;; (lispy-mode -1)
  )

(add-hook 'edebug-setup-hook 'cda-edebug-hook)
