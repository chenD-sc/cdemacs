...
(when (file-exists-p "/home/.ecryptfs/")
  (let ((db-fname (expand-file-name "~/.local/mlocate.db")))
    ...
    (global-set-key (kbd "C-x l") 'cda-counsel-locate)))
...
(define-key ivy-occur-grep-mode-map (kbd "M-i") #'cda-ivy-occur-grep-iedit)
(setq swiper-verbose nil)

(defun cda-ivy-occur-grep-iedit ()
  (interactive)
