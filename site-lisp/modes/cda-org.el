...
(use-package worf
  ...
  (setq worf-visit-function
        (defun cda-worf-visit-function (file)
...
(define-key org-mode-map (kbd "C-M-i") 'cda-org-complete-symbol)
...
(define-key org-mode-map (kbd "C-a") 'cda-move-beginning-of-line)
...
(cda-advice-add 'org-edit-src-exit :after (lambda (&rest _) (save-buffer)))
...
(setq appt-disp-window-function #'cda-appt-display)
(run-at-time "24:01" nil #'cda-org-agenda-to-appt)
(remove-hook 'org-finalize-agenda-hook #'cda-org-agenda-to-appt)
(add-hook 'org-finalize-agenda-hook #'cda-org-agenda-to-appt)
...
(defun cda-org-schedule-today ()
  (interactive)
  (org-agenda-schedule 0 "+0d"))

(defun cda-org-schedule-tomorrow ()
  (interactive)
  (org-agenda-schedule 0 "+1d"))

(defun cda-org-popup (title msg &optional icon sound)
...
(defun cda-appt-display (min-to-app new-time msg)
  "our little façade-function for cda-org-popup"
  (cda-org-popup (format "Appointment in %s minute(s)" min-to-app) msg
                 "/usr/share/icons/gnome/32x32/status/appointment-soon.png"
                 "/usr/share/sounds/ubuntu/stereo/phone-incoming-call.ogg"))

(defun cda-org-agenda-to-appt ()
  "Erase all reminders and rebuild reminders for today from the agenda"
  (interactive)
  ;; (setq appt-time-msg-list nil)
  (org-agenda-to-appt))

(defvar cda-org-structure-template-alist
  '(("s" "#+begin_src ?\n\n#+end_src")
    ("e" "#+begin_example\n?\n#+end_example")
    ("q" "#+begin_quote\n?\n#+end_quote")
    ("v" "#+begin_verse\n?\n#+end_verse")
...
(defun hot-expand (str)
  "Expand org template."
  (let ((beg (point))
        (expansion (cadr (assoc str cda-org-structure-template-alist))))
    (insert expansion)
...
(defhydra hydra-org-objects (:exit t)
  ...
  ("a" cda-org-add-to-agenda "add to agenda")
  ("p" org-pomodoro "pomodoro"))

(defun cda-org-add-to-agenda ()
  (interactive)
  ...
  (let ((str (nth 4 (org-heading-components))))
    (if (not (stringp str))
      ...
      (mapconcat (lambda (s) (substring s 0 (min (length s) 2)))
                 (split-string str "[- =]+")
                 "-"))))
(setq org-clock-heading-function #'cda-org-clock-heading)
...
(defvar cda-snippet-alist
  '((sh-mode . "~/Dropbox/org/wiki/bash.org")))

(defun cda-insert-snippet-action (x)
  (let ((body (cdr x)))
...
(defun cda-insert-snippet ()
  (interactive)
  (let ((file (cdr (assoc major-mode cda-snippet-alist))))
    (if (not (file-exists-p file))
      ...
      (let* ((parse-tree (org-parser-parse-file file))
        ...
        (ivy-read "snippet: " snippet-names-bodies
                  :action 'cda-insert-snippet-action)))))

(defun cda-org-complete-symbol ()
  (interactive)
...
(defun cda-org-grep (s)
  (let ((default-directory emacs-d))
    (counsel-rg s)))

(let ((inhibit-message t))
  (org-add-link-type "grep" 'cda-org-grep))

(defun cda-org-done-p ()
  (save-excursion
    (org-back-to-heading)
    (looking-at "\\*+ \\(DONE\\|CANCELLED\\)")))

(defun cda-org-delete-element (&optional context)
  (setq context (or context (org-element-context)))
  (delete-region
   (org-element-property :begin context)
   (1+ (org-element-property :end context))))

(defun cda-org-cleanup-links ()
  (interactive)
  (save-excursion
    (goto-char (point-min))
