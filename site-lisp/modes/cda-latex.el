...
(define-key LaTeX-mode-map [f5] 'cda-pdflatex-quiet)
...
(mapc
 (lambda (x)
   (font-lock-add-keywords
    'latex-mode
    (cda-fontify-glyph (concat "\\\\" (car x) " ") (cadr x))))
 '(("partial" "∂")
   ("nabla" "∇")
   ("varphi" "φ")
   ("varepsilon" "ε")
...
(defadvice TeX-source-correlate-sync-source (after cda-raise-tex activate)
  (require 'org-fu)
  (orfu-raise-frame))
