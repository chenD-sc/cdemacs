...
(defun cda-set-font (&optional frame)
  (when frame
    (select-frame frame))
  (condition-case nil
      (set-frame-font "WenQuanYi Micro Hei Mono")
    (error
     (ignore-errors
...
(defun cda-advice-add (&rest args)
...
;; (autoload 'mu4e "cda-mu4e")
;; (autoload 'mu4e-compose-new "cda-mu4e")
...
(cda-advice-add 'semantic-idle-scheduler-function :around #'ignore)
(require 'server)
(setq cda-startup-time-toc (current-time))
(or (server-running-p) (server-start))
(setq cda-startup-time-seconds
      (time-to-seconds (time-subtract cda-startup-time-toc cda-startup-time-tic)))
